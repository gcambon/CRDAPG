...
#my $cruise   = 'pirata-fr31';



# TODOS: get path from filename
my $cruise   = $ENV{CRUISE};
my $drive    = $ENV{DRIVE};
my $fileName = $ENV{CRUISE_FORMAT};  #"fr32%03d";
...
for( my $i = $first; $i <= $last; $i++ ) {
  ...
  if( -e $hex ) {
    ...
    if (defined $report ) {
		printf STDERR $path . "\n";
		printf STDERR $station . "\n";
      $cmd = sprintf("ConReport.exe %s\\data\\raw\\%s.xmlcon %s\\data\\reports", $path, $station, $path);
      defined $DEBUG ? printf STDERR "$cmd\n" : system($cmd);
    }    
    if (defined $btl ) {
