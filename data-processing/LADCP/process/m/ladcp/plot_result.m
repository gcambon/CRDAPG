...
if isnan(p.plot_range(4))
  p.plot_range(4)   = zpmax;
end
...
if isfield(dr,'u_sadcp2')
  plot(dr.u_sadcp2*100,dr.z_sadcp2,'-m','linewidth',1.3)
  plot(dr.v_sadcp2*100,dr.z_sadcp2,'-c','linewidth',1.3)
  plot(dr.u_sadcp2*100,dr.z_sadcp2,'pm','linewidth',0.9)
  plot(dr.v_sadcp2*100,dr.z_sadcp2,'pc','linewidth',0.9)
  ct    = [ct,'; pentagon SADCP'];
end
if isfield(dr,'u_dvl')
  plot(dr.u_dvl*100,dr.z_dvl,'--r','linewidth',1.3)
  plot(dr.v_dvl*100,dr.z_dvl,'--g','linewidth',1.3)
  plot(dr.u_dvl*100,dr.z_dvl,'sr','linewidth',0.9)
  plot(dr.v_dvl*100,dr.z_dvl,'sg','linewidth',0.9)
  ct    = [ct,'; pentagon DVL'];
end
...
if isfield(dr,'onlyshear')
  if dr.onlyshear
    text(ax(1),ax(4)*0.95,'  SHEAR SOLUTION ONLY','color','b','fontsize',12)
  end
end
if ps.up_dn_looker==2
  text(ax(1),ax(4)*0.90,'  DOWN LOOKER ONLY','color','b','fontsize',10)
end
if ps.up_dn_looker==3
  text(ax(1),ax(4)*0.90,'    UP LOOKER ONLY','color','b','fontsize',10)
end
plot([0 0],ax(3:4),'-k')
axis(ax)
...
if (p.btrk_used>0 && isfield(dr,'zbot') )
  plot(p.plot_range(1:2),[1 1]*abs(p.plot_range(4)),'-k','linewidth',2)
  set(gca,'XTickLabel',[]);
...
iy          = iy-idy;
text(ix,iy,sprintf('u-mean: %4.1f cm/s    v-mean: %4.1f cm/s',...
...
if ps.sadcpfac2~=0
  dum = [dum,' sad2:',num3str(ps.sadcpfac2,3,1)]; 
end
if ps.dvlfac~=0
  dum = [dum,' dvl:',num3str(ps.dvlfac,3,1)]; 
end
text(ix,iy,dum)
...
axis(ax)
title('range [m]','color','r','fontweight','normal')
...
if isfield(dr,'ensemble_vel_err')==1
  ...
  axis(ax)
  title('single ping','color','b','fontweight','normal')
end
...
%
%subplot(326)
axes('position',[0.58 0.1 0.34 .23])
xctd    = dr.xctd;
yctd    = dr.yctd;
