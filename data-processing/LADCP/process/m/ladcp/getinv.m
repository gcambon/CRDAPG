...
%
% add second ship adcp constraint
%
if isfield(di,'svel2') &&~isempty(di.svel2)
    if sum(isfinite(di.svel(:,1)))>2
        [p,Aocean,A1,d,ds,messages] =...
            lainsadcp2(p,di.svel2,Aocean,A1,d,ps.dz,ps.sadcpfac2,ps.velerr,messages);
        if length(ds.z_sadcp2)>1
            dr.z_sadcp2 = di.svel2(:,1);
            dr.u_sadcp2 = di.svel2(:,2);
            dr.v_sadcp2 = di.svel2(:,3);
            dr.uerr_sadcp2 = di.svel2(:,4);
            % make length of array unique
            if isfield(dr,'zbot')
                lzbot = length(dr.zbot);
            else
            ...
            while length(dr.z_sadcp2)==nt || length(dr.z_sadcp2)==nz || ...
                    length(dr.z_sadcp2)==lzbot
                dr.z_sadcp2    = [dr.z_sadcp2;nan];
                dr.u_sadcp2    = [dr.u_sadcp2;nan];
                dr.v_sadcp2    = [dr.v_sadcp2;nan];
                dr.uerr_sadcp2 = [dr.uerr_sadcp2;nan];
            end
        end
        if ps.sadcpfac2>0
            disp(['    Weight for second SADCP vel is (ps.sadcpfac) ',num2str(ps.sadcpfac2)])
        else
            disp('    Second SADCP not used')
        end
    else
        disp('    Not enough second SADCP data')
        ps.sadcpfac2 = 0;
    end
else
    disp('    No second SADCP data')
    ps.sadcpfac2   = 0;
end

%
% add ship dvl constraint
%

if isfield(di,'dvel') && ~isempty(di.dvel)
    if sum(isfinite(di.dvel(:,1)))>2
        [p,Aocean,A1,d,ds,messages] =...
            laindvl(p,di.dvel,Aocean,A1,d,ps.dz,ps.dvlfac,ps.velerr,messages);
        if length(ds.z_dvl)>1
            dr.z_dvl    = di.dvel(:,1);
            dr.u_dvl    = di.dvel(:,2);
            dr.v_dvl    = di.dvel(:,3);
            dr.uerr_dvl = di.dvel(:,4);
            % make length of array unique
            if isfield(dr,'zbot')
                lzbot = length(dr.zbot);
            else
            ...
            while length(dr.z_dvl)==nt || length(dr.z_dvl)==nz || ...
                    length(dr.z_dvl)==lzbot
                dr.z_dvl    = [dr.z_dvl;nan];
                dr.u_dvl    = [dr.u_dvl;nan];
                dr.v_dvl    = [dr.v_dvl;nan];
                dr.uerr_dvl = [dr.uerr_dvl;nan];
            end
        end
        if ps.dvlfac>0
            disp(['    Weight for DVL vel is (ps.dvlfac) ',num2str(ps.dvlfac)])
        else
            disp('    DVL not used')
        end
    else
        disp('    Not enough DVL data')
        ps.dvlfac = 0;
    end
else
    disp('    No DVL data')
...
if p.use_buc
    if ~isempty(di.u_buc)
        if sum(isfinite(di.u_buc))>2
            if ps.bucfac>0
                [p,Aocean,A1,d,ds,messages] = ...
                    lainbuc(p,di,Aocean,A1,d,ps.dz,ps.bucfac,messages);
                disp(['    Weight for BUC vel is (ps.bucfac) ',num2str(ps.bucfac)])
            else
                disp('    BUC not used')
            end
            dr.u_buc    = di.u_buc;
            dr.v_buc    = di.v_buc;
            dr.dist_buc = di.dist_buc;
            dr.lon_buc  = di.lon_buc;
            dr.lat_buc  = di.lat_buc;
        else
            disp('    Not enough BUC data')
            ps.bucfac = 0;
        end
    else
        disp('    No BUC data')
...
de.ocean_constraints = [de.ocean_constraints;...
    sum(abs(Aocean))-sum(de.ocean_constraints)];
de.ctd_constraints   = [de.ctd_constraints;sum(abs(A1))-sum(de.ctd_constraints)];
de.type_constraints  = [de.type_constraints;'BUC       '];
...
if (psbot==0 && ps.barofac==0 && ps.sadcpfac==0 && ps.sadcpfac2==0 && ps.dvlfac==0)
    disp('    No bottom no barotropic no SADCPs no DVL constraint => setting mean U,V to zero')
    dr.onlyshear = 1;
...
if isfield(values,'bucdata') && values.bucdata
    plot(tim,dr.u_buc,'r','linewidth',1.8)
end
...
if isfield(values,'bucdata') && values.bucdata
    title('ctd(-b) ship(-g) buc(-r) ocean(-k)')
else
...
if isfield(values,'bucdata') && values.bucdata
    plot(tim,dr.v_buc,'r','linewidth',1.8)
end
...
if sum(shipvel) ~= prod(shipvel)
    ...
    if isfield(values,'bucdata') && values.bucdata
        hold on
        plot(tim,dr.dist_buc,'r','linewidth',2)
        hold off
        ylabel('Distance from ship [m]')
        legend('CTD','BUC','position','northwest')
    else
...
plot(xctd(ib),yctd(ib),'g+','markersize',9,'linewidth',2)
if isfield(dr,'xship')
    ...
    if isfield(values,'bucdata') && values.bucdata
        plot(dr.lon_buc+dr.xship,dr.lat_buc+dr.yship,'linewidth',2,'color','r')
        plot(xctd(ib),yctd(ib),'g+','markersize',9,'linewidth',2)
        plot(dr.lon_buc(ii)+dr.xship(ii),dr.lat_buc(ii)+dr.yship(ii),'k.','markersize',10)
        plot([dr.lon_buc(ii)+dr.xship(ii);dr.xship(ii)],[dr.lat_buc(ii)+dr.yship(ii); dr.yship(ii)],'-c','linewidth',0.5)
        xlabel('CTD-position (blue), BUC (red) and ship (green) east-west [m]')
    else
...
function [p,Aocean,Ac,d,ds,messages] = ...
    lainbuc(p,bucvel,Aocean,Ac,d,dz,bucfac,messages)
% function [p,Aocean,Ac,d,ds,messages]=lainbuc(p,bucvel,Aocean,Ac,d,dz,bucfac,messages)
%
% add BUC to the two parts of the A matrix
%
% bucvel = BUC velocity data
%
% d = cell velocity data
% Aocean = ocean matrix
% Ac = ctd matrix
% bucfac = weight for BUC velocity
%
[li,ljo] = size(Ac);
...
%
% extract finite BUC velocities and store in variables
%
dbuc  = bucvel.u_buc+1i*bucvel.v_buc;
ind   = find(isfinite(dbuc));
dbuc  = dbuc(ind);
...
%
if bucfac>0
    fac = sqrt(full(sum(abs(Ac))));
    for n = 1 : length(dbuc)
        Ac(li+n,ind(n)) = bucfac*fac(n);
        d               = [d;-dbuc(n)*bucfac.*fac(n)];
    end
    Aocean(li:length(d),:) = 0;
    disp(['    Mean BUC weight : ',num2str(mean(bucfac.*fac))])
end

iok = find(fac>0.1);
if length(iok) < length(fac)
    fprintf(...
        '    %d out of %d BUC profiles removed because of low weight',...
        length(fac)-length(iok),length(fac));
end

if isempty(iok)
    warn = ('All BUC values removed because of low weight');
    messages.warnp = strvcat(messages.warnp,warn);
    disp(['>   ' warn]);
end
...
if sadcpfac>0
    ...
    for n = 1 : length(dsadcp)
        ...
        jz              = min(max(jz,1),ljo);
        % fac(n)=fac(n)*fac2(jz);
        % variation of the weight with depth
        z_w_vec         = linspace(exp(1),1,round(max(zsadcp)));
        z_w_fac         = log(z_w_vec(1+round(zsadcp(n)-min(zsadcp))));
...
%-------------------------------------------------------------------
function [p,Aocean,Ac,d,ds,messages] = ...
    lainsadcp2(p,svel2,Aocean,Ac,d,dz,sadcpfac2,velerr,messages)
% function [p,Aocean,Ac,d,ds,messages]=lainsadcp(p,svel,Aocean,Ac,d,dz,sadcpfac,velerr,messages)
%
% add SADCP to the two parts of the A matrix
%
...
if sadcpfac2>0
    ...
    for n = 1 : length(dsadcp)
        ...
        z_w_vec         = linspace(exp(1),1,round(max(zsadcp)));
        z_w_fac         = log(z_w_vec(1+round(zsadcp(n)-min(zsadcp))));
        Aocean(li+n,jz) = sadcpfac2*fac(n);
        d               = [d;dsadcp(n)*sadcpfac2.*fac(n)];
    end
    Ac(length(d),1) = 0;
    disp(['    Mean second SADCP weight : ',num2str(mean(sadcpfac2.*fac))])
end

iok = find(fac>0.1);
if length(iok) < length(fac)
    disp(sprintf(...
        '    %d out of %d second SADCP profiles removed because of low weight',...
        length(fac)-length(iok),length(fac)));
end
if isempty(iok)
    warn = ('All second SADCP values removed because of low weight');
    messages.warnp = strvcat(messages.warnp,warn);
    disp(['>   ' warn]);
end

ds.z_sadcp2    = zsadcp(iok);
ds.u_sadcp2    = real(dsadcp(iok));
ds.v_sadcp2    = imag(dsadcp(iok));
ds.uerr_sadcp2 = verr(iok);

%-------------------------------------------------------------------
function [p,Aocean,Ac,d,ds,messages] = ...
    laindvl(p,dvel,Aocean,Ac,d,dz,dvlfac,velerr,messages)
% function [p,Aocean,Ac,d,ds,messages]=laindvl(p,dvel,Aocean,Ac,d,dz,dvlfac,velerr,messages)
%
% add DVL to the two parts of the A matrix
%
% dvel = DVL data velocity profile
%
% d = cell velocity data
% Aocean = ocean matrix
% Ac = ctd matrix
% dvlfac = weight for DVL velocity
% velerr = velocity error for LADCP super ensemble
%
[li,ljo] = size(Aocean);
...
%
% extract finite DVL velocities and store in variables
%
zdvl    = abs(dvel(:,1));
ddvl    = dvel(:,2)+1i*dvel(:,3);
ind     = find(isfinite(ddvl));
zdvl    = zdvl(ind);
ddvl    = ddvl(ind);
verr    = dvel(ind,4);
...
%
% weight according to scatter in mean DVL profile
% first replace small or no error with large error
%
ind     = find(~isfinite(verr) | verr<0.01);
verr(ind) = nmax(verr);
...
if dvlfac>0
    ...
    for n = 1 : length(ddvl)
        ...
        % fac(n)=fac(n)*fac2(jz);
        % variation of the weight with depth
        z_w_vec         = linspace(exp(1),1,round(max(zdvl)));
        z_w_fac         = log(z_w_vec(1+round(zdvl(n)-min(zdvl))));
        Aocean(li+n,jz) = dvlfac*fac(n);
        d               = [d;ddvl(n)*dvlfac.*fac(n)];
    end
    Ac(length(d),1) = 0;
    disp(['    Mean DVL weight : ',num2str(mean(dvlfac.*fac))])
end

iok = find(fac>0.1);
if length(iok) < length(fac)
    disp(sprintf(...
        '    %d out of %d DVL profiles removed because of low weight',...
        length(fac)-length(iok),length(fac)));
end
if isempty(iok)
    warn = ('All DVL values removed because of low weight');
    messages.warnp = strvcat(messages.warnp,warn);
    disp(['>   ' warn]);
end

ds.z_dvl    = zdvl(iok);
ds.u_dvl    = real(ddvl(iok));
ds.v_dvl    = imag(ddvl(iok));
ds.uerr_dvl = verr(iok);
