...
if ~isempty(p.edit_mask_dn_bins) || ~isempty(p.edit_mask_up_bins)
  ...
  fprintf('    Bin masking               : set %d weights to NaN\n',nbad);
...
% same as above, changing to only down, GK Sep 2007
  ...
  fprintf('    Side-lobe contamination   : set %d weights to NaN\n',nbad);
...
if p.edit_PPI
  ...
  fprintf('    Previous-ping interference: set %d weights to NaN\n',nbad);
...
if ~isempty(p.edit_skip_ensembles)
  ...
  fprintf('    Ensemble skipping         : set %d weights to NaN\n',nskipped);
...
%      end
  ...
  fprintf('    Last Bin masking           : set %d weights to NaN\n',nbad);
...
if any(p.minimum_correlation_threshold>0)
  ...
  fprintf('    Minimum corr. threshold   : set %d weights to NaN\n',nbad);
end
