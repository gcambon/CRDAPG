...
if ~isempty(data.tim_sadcp2)
  disp('    2nd SADCP data coverage:')
  disp(['      ',datestr(gregoria(data.tim_sadcp2(1)),31),'  to  ',...
               datestr(gregoria(data.tim_sadcp2(end)),31)])
end
if ~isempty(data.tim_dvl)
  disp('    DVL data coverage:')
  disp(['      ',datestr(gregoria(data.tim_dvl(1)),31),'  to  ',...
               datestr(gregoria(data.tim_dvl(end)),31)])
end
if ~isempty(data.tim_buc)
  disp('    BUC data coverage:')
  disp(['      ',datestr(gregoria(data.tim_buc(1)),31),'  to  ',...
               datestr(gregoria(data.tim_buc(end)),31)])
end
...
if ~isempty(data.ctdtime_data)
  ...
  if min(data.ctdtime_time)>max(data.time_jul) ||...
    ...
    % create a w_CTD time series with roughly the same time stepping as the 
    % LADCP this needs to be done to avoid aliasing by a more highly
...
%    wctd = wctd(ind);
    ...
    w_up   = meanmediannan(data.rw(data.izu,:),2);
    w_dn   = meanmediannan(data.rw(data.izd,:),2);
...
%    ii = [ii, [-nsamples:nsamples]+fix(length(i1)*0.7)];
    ...
    xlabel('sample scans: ADCP down (b) / ADCP up (r) / CTD (k)')
    grid
