...
if min(d.time_jul)<tim(1) || max(d.time_jul)>tim(end)
    ...
    if min(d.time_jul)<tim(1)
        dt       = 86400*(tim(1)-min(d.time_jul));
        disp(['>   using constant uctd to extrapolate at the beginning of the profile over ',num2str(fix(dt)),' seconds']);
        tim      = tim([1,1:end]);
    ...
    if max(d.time_jul)>tim(end)
        dt       = max(d.time_jul)-tim(end);
        disp(['>   using constant uctd to extrapolate at the end of the profile over ',num2str(fix(dt)),' seconds']);
        tim      = tim([1:end,end]);
...
itm             = meshgrid([1:it],[1:ib]);
% each of ib rows of itm contains 1:it
...
ij              = find( iz>0 & iz<max(dr.z)/dz);
% ij contains same as ii but also for
% invalid velocities
...
if iplot
    ...
    colorbar('h','position',[pos(1) pos(2)-.06 pos(3) .01]);
    ...
    grid
    set(gca,'Ydir','reverse');
    title('V (-k) shear (-r) inverse (-g)\pm std','interpreter','tex')
    xlabel('velocity [m/s]')
