...
% check second SADCP
if isfield(dr,'z_sadcp2')

  % std of missmatch between SADCP and LADCP velocity profile
  us        = interp1(dr.z,dr.u,dr.z_sadcp2);
  vs        = interp1(dr.z,dr.v,dr.z_sadcp2);
  usso      = ([us-dr.u_sadcp2]);
  vsso      = ([vs-dr.v_sadcp2]);
  uvsso     = nmean(sqrt(usso.^2+vsso.^2));
 
  % given std of SADCP profile
  uvssi     = nmedian(dr.uerr_sadcp2);

  disp(['    Check second SADCP        rms: ',num3str(uvsso,6,3),...
       '  should be smaller than ',num3str(uvssi,6,3),' / ',num3str(ps.sadcpfac2,6,3)])
...
if isfield(dr,'z_dvl')
  ...
  % std of missmatch between DVL and LADCP velocity profile
  ud        = interp1(dr.z,dr.u,dr.z_dvl);
  vd        = interp1(dr.z,dr.v,dr.z_dvl);
  udso      = ([ud-dr.u_dvl]);
  vdso      = ([vd-dr.v_dvl]);
  uvdso     = nmean(sqrt(udso.^2+vdso.^2));
 
  % given std of DVL profile
  uvdsi     = nmedian(dr.uerr_dvl);

  disp(['    Check DVL        rms: ',num3str(uvdso,6,3),...
       '  should be smaller than ',num3str(uvdsi,6,3),' / ',num3str(ps.dvlfac,6,3)])
