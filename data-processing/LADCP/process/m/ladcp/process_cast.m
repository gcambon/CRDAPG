function [fig1] = process_cast(cfg,extraarg)
...
if size(messages.warn,1) + size(messages.warnp,1) > 2
    ...
    for j=1:size(messages.warn,1)
        text(0,1.1-j/10,messages.warn(j,:),'color','r','fontsize',11,'fontweight','bold')
    end
    for j=1:size(messages.warnp,1)
        text(0,1.1-(size(messages.warn,1)+1+j)/10,messages.warnp(j,:),'color','r','fontsize',11,'fontweight','bold')
    end
    axis off
else
    text(0,1.1-1/10,'LADCP profile OK','color','g','fontsize',20,'fontweight','bold')
    axis off
end
