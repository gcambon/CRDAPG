...
if dd.Coordinates==0
    ...
    for ii = 1:length(veld)
        veld(ii,:,:) = beam2earth(squeeze(veld(ii,:,:)),vd(ii,:,:),dd);%(beam, head, HeadingOffset, pitch, roll, ssnd, ECssnd, beams_up, xfreq, convex, sensor_config, BeamAngle);
    end
end
...
if values.up==1
    ...
    if du.Coordinates==0
        ...
        for ii = 1:length(veld)
            velu(ii,:,:) = beam2earth(squeeze(velu(ii,:,:)),vu(ii,:,:),du);%(beam, head, HeadingOffset, pitch, roll, ssnd, ECssnd, beams_up, xfreq, convex, sensor_config, BeamAngle);
        end
    ...
    ind       = (pgu < pglim);
    if any(ind)
        fprintf('    Removed %d uplooker values because of percent good < %g\n',...
            length(find(ind)),pglim);
    end
    pgu(ind)  = NaN;
    pgu(~ind) = 1;
...
if values.up==1
    ...
    if vu(1,1,v.tim)-vd(1,1,v.tim)>10/86400
        disp('>   data at beginning of uplooker profile is missing')
    ...
    elseif vd(1,1,v.tim)-vu(1,1,v.tim)>20/86400
        disp('>   data at beginning of downlooker profile is missing')
    ...
    %
    % plotting ping rate diagnostics for inspection
    %
    sfigure(2);
    clf
    
    tmin   = max([timd(1) timu(1)]);
    tmax   = min([timd(end) timu(end)]);
    ind_dn = find(timd>=tmin & timd<=tmax);
    ind_up = find(timu>=tmin & timu<=tmax);
    
    % ping rate for uplooker
    subplot(4,1,1);
    rate        = 86400*diff(timu(ind_up));      % ping rate in seconds
    found_rates = unique(rate);
    nrates      = length(found_rates);
    if nrates==1
        disp(['    Found 1 ping rate for uplooker: ',sprintf('%7.5f',found_rates)]);
    else
        disp(['    Found ',num2str(nrates),' ping rates for uplooker: ',sprintf('%7.5f ',found_rates)]);
    end
    plot(rate,'b.','markersize',8);
    set(gca,'Xlim',[0 length(ind_up)+1],'Ylim',[min(rate)-.1 max(rate)+0.1]);
    grid on
    ylabel('ping rate [s]');
    title(['uplooker ping rate (',num2str(min(rate),'%6.5f'),'s - ',num2str(max(rate),'%6.5f'),'s)']);
    
    % ping rate for downlooker
    subplot(4,1,2);
    rate        = 86400*diff(timd(ind_dn));	% ping rate in seconds
    found_rates = unique(rate);
    nrates      = length(found_rates);
    if nrates==1
        disp(['    Found 1 ping rate for downlooker: ',sprintf('%7.5f',found_rates)]);
    else
        disp(['    Found ',num2str(nrates),' ping rates for downlooker: ',sprintf('%7.5f ',found_rates)]);
    end
    plot(rate,'b.','markersize',8);
    set(gca,'Xlim',[0 length(ind_dn)+1],'Ylim',[min(rate)-.1 max(rate)+0.1]);
    grid on
    ylabel('ping rate [s]');
    title(['downlooker ping rate (',num2str(min(rate),'%6.5f'),'s - ',num2str(max(rate),'%6.5f'),'s)']);
    
    % detection of possible interactions
    subplot(4,1,3);
    diff_time_dn = NaN*timd;
    for it = 1:length(timd);
        ind = find(timu>=timd(it));
        if ~isempty(ind)
            diff_time_dn(it) = timu(ind(1))-timd(it);
        end
    end
    diff_time_up = NaN*timu;
    for it = 1:length(timu);
        ind = find(timd>=timu(it));
        if ~isempty(ind)
            diff_time_up(it) = timd(ind(1))-timu(it);
        end
    end
    plot(1440*(timd-tmin),-86400*diff_time_dn,'b.');
    hold on
    plot(1440*(timu-tmin),+86400*diff_time_up,'r.');
    hold off
    set(gca,'Xlim',1440*[0 tmax-tmin],'Ylim',[-0.5 0.5]);
    grid on
    xlabel('time [min]');
    ylabel('time difference [s]');
    title('time between pings: up after down (blue) / down after up (red)');
    
    % time difference between up and down
    subplot(4,1,4);
    ntime  = min([length(timu) length(timd)]);
    diff_t = 86400*(timu(1:ntime)-timd(1:ntime));
    plot(1440*(timd(1:ntime)-tmin),diff_t,'k-');
    set(gca,'Xlim',1440*[0 tmax-tmin]);
    grid on
    ylabel('time difference [s]');
    title('time between up- and down-looker clocks');
    
    suplabel([params.name,' Figure 17'],'t');
    hgsave('tmp/17');
...
%%%%%% Old version
%               disp(['>   Resampling up instrument to down instrument''s
%               timing.'])
%       iu = [1:length(timd)];
...
%       id = [1:length(timd)];
    ...
    if params.up2down==2
        [lag,iu,iid,co] = bestlag(wb2u,wb2d);
    else
        [lag,iiu,id,co] = bestlag(wb2u,wb2d);
    end
