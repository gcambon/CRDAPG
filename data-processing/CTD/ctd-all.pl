...
my ( $julien, $h_date, $f_date, $date, $mois, $jour, $annee, $heure, $min, $sec, $time );
my ($year,$month,$day,$hour);
my $type_cast;

my $meta
  ;    # reference anonyme vers la table hashage des coefficients de calibration
my $primary   = 'primary';
...
sub usage() {
  ...
  print STDERR
    "    --begin_date=JJ/MM/YYYY     starting date from cycle_mesure\n";
  ...
  print STDERR
    "    --ctd_all              output primary and secondary sensors\n";
  print STDERR
"    --top                  complement surface data with first bin values\n";
  ...
  while (<DATA_CON>) {
    ...
    switch ($.) {
      ...
      case 8 {
        (
          undef, undef,
          $meta->{'cond'}->{'secondary'}->{'slope'},
          $meta->{'cond'}->{'secondary'}->{'offset'}
        ) = split;
      }
      case 112 {
        (
          $meta->{'cond'}->{'secondary'}->{'g'},
          $meta->{'cond'}->{'secondary'}->{'h'},
          $meta->{'cond'}->{'secondary'}->{'i'},
          $meta->{'cond'}->{'secondary'}->{'j'},
          $meta->{'cond'}->{'secondary'}->{'ctcor'},
          $meta->{'cond'}->{'secondary'}->{'cpcor'}
        ) = split;
      }
      ...
      case 3 {
        (
          undef, undef,
          $meta->{'cond'}->{'primary'}->{'primary'}->{'slope'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'offset'}
        ) = split;
      }
      case 110 {
        (
          $meta->{'cond'}->{'primary'}->{'primary'}->{'g'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'h'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'i'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'j'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'ctcor'},
          $meta->{'cond'}->{'primary'}->{'primary'}->{'cpcor'}
        ) = split;
      }
      ...
      case 5 {
        (
          undef, undef, undef, undef, undef,
          $meta->{'temp'}->{'primary'}->{'slope'},
          $meta->{'temp'}->{'primary'}->{'offset'}
        ) = split;
      }
      case 111 {
        (
          $meta->{'temp'}->{'primary'}->{'f0'},
          $meta->{'temp'}->{'primary'}->{'g'},
          $meta->{'temp'}->{'primary'}->{'h'},
          $meta->{'temp'}->{'primary'}->{'i'},
          $meta->{'temp'}->{'primary'}->{'j'}
...
sub get_xmlcon_sbe03 {
  ...
  $meta->{'temp'}->{$sensor}->{'sn'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/SerialNumber"
  );
  $meta->{'temp'}->{$sensor}->{'date'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/CalibrationDate"
  );
  $meta->{'temp'}->{$sensor}->{'g'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/G"
  );
  $meta->{'temp'}->{$sensor}->{'h'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/H"
  );
  $meta->{'temp'}->{$sensor}->{'i'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/I"
  );
  $meta->{'temp'}->{$sensor}->{'j'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/J"
  );
  $meta->{'temp'}->{$sensor}->{'f0'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/F0"
  );
  $meta->{'temp'}->{$sensor}->{'slope'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /TemperatureSensor/Slope"
  );
  $meta->{'temp'}->{$sensor}->{'offset'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
...
sub get_xmlcon_sbe04 {
  ...
  $meta->{'cond'}->{$sensor}->{'sn'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/SerialNumber"
  );
  $meta->{'cond'}->{$sensor}->{'date'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/CalibrationDate"
  );
  $meta->{'cond'}->{$sensor}->{'g'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients/G"
  );
  $meta->{'cond'}->{$sensor}->{'h'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients/H"
  );
  $meta->{'cond'}->{$sensor}->{'i'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients/I"
  );
  $meta->{'cond'}->{$sensor}->{'j'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients/J"
  );
  $meta->{'cond'}->{$sensor}->{'cpcor'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients[\@equation=\'1\']/CPcor"
  );
  $meta->{'cond'}->{$sensor}->{'ctcor'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Coefficients[\@equation=\'1\']/CTcor"
  );
  $meta->{'cond'}->{$sensor}->{'slope'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /ConductivitySensor/Slope"
  );
  $meta->{'cond'}->{$sensor}->{'offset'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
...
sub get_xmlcon_pressure {
  ...
  $meta->{'press'}->{$sensor}->{'sn'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/SerialNumber"
  );
  $meta->{'press'}->{$sensor}->{'date'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/CalibrationDate"
  );
  $meta->{'press'}->{$sensor}->{'c1'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/C1"
  );
  $meta->{'press'}->{$sensor}->{'c2'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/C2"
  );
  $meta->{'press'}->{$sensor}->{'c3'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/C3"
  );
  $meta->{'press'}->{$sensor}->{'d1'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/D1"
  );
  $meta->{'press'}->{$sensor}->{'d2'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/D2"
  );
  $meta->{'press'}->{$sensor}->{'t1'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/T1"
  );
  $meta->{'press'}->{$sensor}->{'t2'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/T2"
  );
  $meta->{'press'}->{$sensor}->{'t3'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/T3"
  );
  $meta->{'press'}->{$sensor}->{'t4'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/T4"
  );
  $meta->{'press'}->{$sensor}->{'t5'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/T5"
  );
  $meta->{'press'}->{$sensor}->{'ad590m'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/AD590M"
  );
  $meta->{'press'}->{$sensor}->{'ad590b'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/AD590B"
  );
  $meta->{'press'}->{$sensor}->{'slope'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
    /PressureSensor/Slope"
  );
  $meta->{'press'}->{$sensor}->{'offset'} = &get_xml_info(
    $root,
"/SBE_InstrumentConfiguration/Instrument/SensorArray/Sensor[\@index=\'$index\'] \
...
sub decode_xmlcon_file {
  ...
  $acquisitionVersion =
    &get_xml_info( $root,
    '/SBE_InstrumentConfiguration/@SB_ConfigCTD_FileVersion' );
  $ctdType =
...
sub write_xml_oceano_temperature {
  ...
  print XML_CTD_FILE
"    <INSTRUMENT TYPE=\"SBE03\" NUMERO_SERIE=\"$meta->{'temp'}->{$sensor}->{'sn'}\" CODE_ROSCOP=\"TEMP\" PERE=\"$ctdSn\" NUMERO=\"$SENSOR\">\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"DATE\" TYPE=\"D\">$meta->{'temp'}->{$sensor}->{'date'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"G\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'g'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"H\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'h'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"I\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'i'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"J\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'j'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"F0\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'f0'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"SLOPE\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'slope'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"OFFSET\" TYPE=\"N\">$meta->{'temp'}->{$sensor}->{'offset'}</PARAMETRE>\n";
...
sub write_xml_oceano_conductivity {
  ...
  print XML_CTD_FILE
"    <INSTRUMENT TYPE=\"SBE04\" NUMERO_SERIE=\"$meta->{'cond'}->{$sensor}->{'sn'}\" CODE_ROSCOP=\"PSAL\" PERE=\"$ctdSn\" NUMERO=\"$SENSOR\">\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"DATE\" TYPE=\"D\">$meta->{'cond'}->{$sensor}->{'date'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"G\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'g'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"H\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'h'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"I\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'i'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"J\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'j'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"CTCOR\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'ctcor'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"CPCOR\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'cpcor'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"SLOPE\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'slope'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"OFFSET\" TYPE=\"N\">$meta->{'cond'}->{$sensor}->{'offset'}</PARAMETRE>\n";
  print XML_CTD_FILE "    </INSTRUMENT>\n";
}
...
sub write_xml_oceano_pressure {
  ...
  print XML_CTD_FILE
"    <INSTRUMENT TYPE=\"PAROSCIENTIFIC\" NUMERO_SERIE=\"$meta->{'press'}->{$sensor}->{'sn'}\" CODE_ROSCOP=\"PRES\" PERE=\"$ctdSn\">\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"DATE\" TYPE=\"D\">$meta->{'press'}->{$sensor}->{'date'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"C1\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'c1'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"C1\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'c2'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"C3\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'c3'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"D1\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'d1'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"D2\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'d2'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"T1\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'t1'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"T1\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'t2'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"T3\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'t3'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"T4\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'t4'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"SLOPE\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'slope'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"OFFSET\" TYPE=\"N\">$meta->{'press'}->{$sensor}->{'offset'}</PARAMETRE>\n";
  print XML_CTD_FILE "    </INSTRUMENT>\n";
}
...
sub write_xml_oceano_oxygen {
  ...
  print XML_CTD_FILE
"    <INSTRUMENT TYPE=\"SBE43\" NUMERO_SERIE=\"$meta->{'o2'}->{$sensor}->{'sn'}\" CODE_ROSCOP=\"PSAL\" PERE=\"$ctdSn\" NUMERO=\"$SENSOR\">\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"DATE\" TYPE=\"D\">$meta->{'o2'}->{$sensor}->{'date'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"SOC\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'soc'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"OFFSET\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'offset'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"A\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'a'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"B\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'b'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"C\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'c'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"D0\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'d0'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"D1\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'d1'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"D2\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'d2'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"E\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'e'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"TAU20\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'tau20'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"H1\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'h1'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"H2\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'h2'}</PARAMETRE>\n";
  print XML_CTD_FILE
"      <PARAMETRE CLE=\"H3\" TYPE=\"N\">$meta->{'o2'}->{$sensor}->{'h3'}</PARAMETRE>\n";
  print XML_CTD_FILE "    </INSTRUMENT>\n";
}
...
"      <VALIDATION LOGICIEL=\"$0\" VERSION=\"$VERSION\" DATE=\"$today\" OPERATEUR=\"$creator\" CODIFICATION=\"OOPC\">\n";
  ...
  print XML_CTD_FILE
    "    <INSTRUMENT TYPE=\"$ctdType\" NUMERO_SERIE=\"$ctdSn\">\n";
  print XML_CTD_FILE "    </INSTRUMENT>\n";
...
sub entete_odv {
  ...
  print ODV_FILE "//\n";
  print ODV_FILE
"Cruise\tStation\tType\tmon/day/yr\thh:mm\tLongitude [degrees_east]\tLatitude [degrees_north]\tBot. Depth [m]";
...
$hdr_file = lc $ascii_dir . $cycle_mesure . $ctdall . '.ctd'
  if defined $xml or $ascii;
$xml_ctd_file = lc $ascii_dir . $cycle_mesure . $ctdall . '_ctd.xml'
  if defined $xml;
$odv_file = lc $odv_dir . $cycle_mesure . $ctdall . '_ctd_odv.txt'
  if defined $odv;
...
$header_file =
  "$cycle_mesure  $plateforme  $institute  $ctdType  $ctdSn  $pi\n";
...
for ( my $i = 0 ; $i <= $#ARGV ; $i++ ) {
  ...
  while (<DATA_FILE>) {
    ...
    if (/System UpLoad Time =\s+(\.*)/) {    # a modifier suivant le contexte
      ...
      $julien =
        &date2julian( $annee, $annee, $mois, $jour, $heure, $min, $sec );
    }
  ...
  for my $file (@files_descr) {
    printf $file "%05d  %d %7.3f %7.4f %8.4f %s %6.5g",
      $PRFL, $code, $julien, $lat_pos, $long_pos,
      &dateFormat( $time, "%Y%m%d%H%M%S" ), $bottom_depth;
  ...
  if ( $fileName =~ /\.HDR/i ) {
    ...
    $fileName =~ s/data\/asc\//data\/cnv\//i;
  }
  $theDepth = 0;
  open( DATA_FILE, $fileName );
  while (<DATA_FILE>) {
    ...
    if ( not( /^[*#]/ || /^\s*$/ ) ) {
      ...
      # extract values and fill hash %data with position gave by hash %split from config.ini
      @data = split /\s+/;
      while ( my ( $key, $value ) = each(%split) ) {
        $data{$key} = $data[$value];
      }
      # if the file is .asc, pass the firsdt line
      next if ( $data{'PRES'} eq 'PrDM' );

      # retrieve only downcast values
      next if ( $data{'PRES'} < $theDepth ); 

      # memorize the last press value from hash
      $theDepth =  $data{'PRES'};
      #print STDERR "$theDepth\n";

      # memorize profil number in hash
      $data{'PRFL'} = $PRFL;
  ...
  # get the end of CTD cast (parse the last ETDD value) for header 
  if ( defined $ascii or $xml) {
    ($year,$month,$day,$hour,$min,$sec) = &julian2date($annee,$data{'ETDD'}-1);
    # conversion en format US (MM/DD/YY) pour ParseDate
    $f_date = sprintf("%02d/%02d/%4d %02d:%02d:%02d", $day, $month, $year, $hour, $min, $sec);
    # remove the path and extension from filename 
    my ( $name, $dir ) = fileparse $fileName;
    $name =~ s/\.cnv//;    
    printf HDR_FILE "%05d %s %s %02d°%05.2f %s %03d°%05.2f %s %5.4g %5.4g %s\n",
      $PRFL,
      $h_date, $f_date, $lat_deg, $lat_min, $lat_hemi, $long_deg, $long_min,
      $long_hemi, $theDepth, $bottom_depth, $name;
  }

  # affiche l'entete profil a l'ecran
  if ( defined $ascii or $xml ) {
    printf STDERR "   %05d %s %s %02d°%05.2f %s %03d°%05.2f %s %5.4g %5.4g", $PRFL,
      $h_date, $f_date, $lat_deg, $lat_min, $lat_hemi, $long_deg, $long_min,
      $long_hemi, $theDepth, $bottom_depth
      if defined $echo;
  }
  ...
  if ( defined $odv && !defined $ascii ) {
    printf STDERR ": %s %05d %s%s %8.4f %7.4f %5.4g %5.4g",
      ...
      $lat_pos, $long_pos, $data{'PRES'}, $bottom_depth
      if defined $echo;
  }
